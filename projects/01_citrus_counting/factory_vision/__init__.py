...
the repository root and three counting projects imported it. A copy of exactly
what this one case needs lives here instead, so the folder can be lifted into a
repository of its own and still run.

``factory_vision.assets``
    Fetches the model weights on first run, with a progress bar, and checks the
    source clip you place in `input/` yourself.

``factory_vision.counting``
    The zero-shot counting engine: detect, track, count a line crossing, render.
